...
import { getAllAuthors, addAuthor, getAuthor, updateAuthor } from '../../../Services/AuthorsService';
...
class EditAuthorPage extends React.Component {
    ...
    handleClick(){
        ...
        if(check){
            updateAuthor(this.state.author).then(data => {
    ...
    componentDidMount(){       
        getAuthor(this.props.match.params.id).then((data) => {
            if(data.statu && data.statu===true){
                this.setState({ author:data.data.author });
            }else{
                this.setState({redirect:true});
            }
        });
            ...
            <div className="container">
                <form style={{border:'1px solid #ccc', padding:'20px', borderRadius:'5px',marginBottom:'150px', Background:'#fff'}} method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text"  className="form-control" placeholder="Author Name" name="author_name" value={this.state.author.author_name} onChange={this.handleChange}/>
                    </div>
                    <div class="form-group">
                        <label>Author Surname: </label> 
                        <input type="text"  className="form-control" placeholder="Author Surname" name="author_surname" value={this.state.author.author_surname} onChange={this.handleChange}/>
                    </div>
                    <button type="button" className="btn btn-primary" onClick={this.handleClick}>Edit Author</button>
                </form>
            </div>
        ]
    }
