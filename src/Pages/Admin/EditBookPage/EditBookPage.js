...
import { updateBook, getBook } from '../../../Services/BooksService';
...
class EditBookPage extends React.Component {
    ...
    handleClick(){
        ...
        if(check){
            updateBook(this.state.book).then(data => {
    ...
    componentDidMount(){  
        ...
        getBook(this.props.match.params.id).then((data) => {
            if(data.statu && data.statu===true){
                this.setState({ book:data.data.book });
            }else{
                this.setState({redirect:true});
            }
        });
            ...
            <div className="container">
                <form style={{border:'1px solid #ccc', padding:'20px', borderRadius:'5px',marginBottom:'150px', Background:'#fff'}} method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text"  className="form-control" placeholder="Book Name" name="book_name" value={this.state.book.book_name} onChange={this.handleChange}/>
                    </div>
                    <div className="form-group">
                        <label>Author: </label> 
                        <select className="form-control" name="author_id" value={this.state.book.author_id} onChange={this.handleChange}>
                            <option value="0">Select an Author</option>
                        {
                            this.state.authors.map(function(item,i){
                                return (
                                    <option key={i} value={item.author_id}>{item.author_name} {item.author_surname}</option>
                                )
                            })
                        }
                        </select>
